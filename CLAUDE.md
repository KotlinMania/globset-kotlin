# Claude Code Project Instructions — globset-kotlin

## Project Overview

This is **globset-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`globset`](https://crates.io/crates/globset).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/globset/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/globset>
